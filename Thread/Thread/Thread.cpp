...
int main()
{
	...
	//Первый поток
	thread thread1{ counter,std::ref(thrd1_sum), a, arr_size, 0, (arr_size / 2) };
	

	//Второй поток
	thread thread2{ counter,std::ref(thrd2_sum), a, arr_size, (arr_size / 2), arr_size };
	

	thread1.join();
	thread2.join();
	cout << "Thrd_sum = " << thrd1_sum+thrd2_sum << "\n";
